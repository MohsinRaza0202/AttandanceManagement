export const STUDENTS_AUTHENTICATE = 'STUDENTS_AUTHENTICATE';
...
export const studentsAuthenticate = (userId, token, expiryTime) => {
  return dispatch => {
    dispatch(SetLogoutTimer(expiryTime));
    dispatch({
      type: STUDENTS_AUTHENTICATE,
      userId: userId,
      token: token
    });
  };
...
export const signup = (email, password) => {
  return async dispatch => {
    ...
    dispatch(studentsAuthenticate(resData.localId, resData.idToken, parseInt(resData.expiresIn) * 1000));
